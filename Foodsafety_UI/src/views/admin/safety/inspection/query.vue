<template>
      ...
      <div class="field-box">
        <el-form :inline="true" :model="listQuery">
          ...
          <el-form-item label="状态"  class="filter-item">
            <el-select  v-model="listQuery.planStatus" placeholder="请选择" clearable filterable>
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading"
      ...
      <el-table-column align="center" label="操作" width="150" fixed="right" v-if="needFixedRight">
        <template slot-scope="scope">
          ...
          <el-button size="mini"  @click="preview(scope.row)">预览
          </el-button>
        </template>
      </el-table-column>
        ...
        <div style="height: 79vh;">
          <el-tabs v-model="tabPosition">
            ...
            <el-tab-pane label="检查信息" v-if="inspectionResultShow">
              ...
              <el-row>
                <el-col :span="24">
                  <template-project :height="height" :templateId="templateId" :existResult="inspectionResultShow" :setProjectResult="setProjectResult" ref="template"></template-project>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="23">
  ...
  <!--日常检查预览对话框（用于下载）-->
    <el-dialog title="预览" width="70%" :visible.sync="previewVisible":close-on-click-modal="false">
      <div slot="title">
        <i class="el-icon-document">预览</i>
      </div>
      <div style="max-height: 700px;overflow: auto">
        <down-load :planId="planId" v-if="previewVisible"></down-load>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  import { pageQury, getInspectionPlan } from "api/admin/safety/inspection/index";
  ...
  export default {
    ...
    components: {
      ...
      "down-load": () => import("./components/download"),
      queryConditions
    },
    data() {
      return {
