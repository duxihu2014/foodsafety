...
export default {
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.listQuery = {page: page, limit: limit, deliveryTime: [], enterpriseId: undefined,  areaId: undefined, gridId: undefined};
    },
    ...
    update(formName) {
      ...
      set[formName].validate(valid => {
        if (valid) {
          updateRecordWaste(this.form.situationId, this.form).then(() => {
            this.dialogFormVisible = false;
            this.getList();
            this.$notify({
              title: '成功',
